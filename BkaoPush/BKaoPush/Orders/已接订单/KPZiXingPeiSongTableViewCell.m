//
//  KPZiXingPeiSongTableViewCell.m
//  BKaoPush
//
...
#import "KPZiXingPeiSongTableViewCell.h"

@implementation KPZiXingPeiSongTableViewCell
{
...
-(void)caeateUI
{
    ...
    self.disanfang = [[UILabel alloc] initWithFrame:CGRectMake(TAP , CGRectGetMaxY(linView4.frame),ScreenWidth-size3.width-60-4*TAP, 15)];
    self.disanfang.font =[UIFont systemFontOfSize:9];
    self.disanfang.textColor =[UIColor colorWithWhite:0.8 alpha:1];
    self.disanfang.textAlignment = NSTextAlignmentLeft;
    self.disanfang.text =@"自行配送";
    [self.contentView addSubview:self.disanfang];
    
    //第三方运费价钱
    
    self.disanjiaqian = [[UILabel alloc] initWithFrame:CGRectMake(TAP , CGRectGetMaxY(self.disanfang.frame),ScreenWidth-size3.width-60-4*TAP, 20)];
    ...
    self.zongjiaLab = [[UILabel alloc] initWithFrame:CGRectMake(ScreenWidth-size3.width-TAP-5,CGRectGetMaxY(linView4.frame), size3.width, 40)];
    ...
    self.zongjiaLab1 = [[UILabel alloc] initWithFrame:CGRectMake(ScreenWidth-size3.width-TAP-50, CGRectGetMaxY(linView4.frame),50, 40)];
    ...
    UIView *linView5 =[[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.zongjiaLab.frame), ScreenWidth, 1)];
    linView5.backgroundColor = [UIColor colorWithWhite:0.95 alpha:1];
    [self.contentView addSubview:linView5];
    
    self.queRenBtn =[[UIButton alloc] initWithFrame:CGRectMake(ScreenWidth-TAP-80, CGRectGetMaxY(linView5.frame)+TAP, 80, 30)];
    [self.queRenBtn setTitle:@"确认送达" forState:UIControlStateNormal];
    self.queRenBtn.titleLabel.textColor = [UIColor blackColor];
    self.queRenBtn.titleLabel.font =SS_FONT;
    self.queRenBtn.backgroundColor =KPYellowColor;
    [self.queRenBtn.layer setMasksToBounds:YES];
    [self.queRenBtn.layer setCornerRadius:5.0];
    [self.queRenBtn addTarget:self action:@selector(queRenBtnAction:) forControlEvents:UIControlEventTouchUpInside];
    
    [self.contentView addSubview:self.queRenBtn];
//
    
    
    self.frame =CGRectMake(0, 0, ScreenWidth, CGRectGetMaxY(self.queRenBtn.frame)+10);
    
}
-(void)queRenBtnAction:(UIButton *)button
{
    [self.delegates KPLYQueRenSongDaAction:button];
